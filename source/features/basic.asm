...
mainloop:
	...
	mov bl, err_syntax			; Otherwise show an error and quit
	jmp error
...
.display_message:
	xor bx, bx				; Others are blank
	xor cx, cx
	xor dx, dx				; One-choice box
	call os_dialog_box
...
.is_ok:
	...
	movzx ax, byte [gosub_depth]		; Get current GOSUB nest level

	cmp al, 9
	jle .within_limit

	mov bl, err_nest_limit
	jmp error
...
.rename:
	...
	xor ax, ax			; It worked sucessfully, so set "R = 0" to indicate no error
	mov byte al, 'R'
	xor bx, bx
	call set_var
...
error:
	mov ax, 5 * 256			; Revert display page
	int 10h
...
.finish:
	...
	err_char_in_num		equ 00h
	err_cmd_unknown		equ 01h
	err_divide_by_zero	equ 02h
	err_doloop_maximum	equ 03h
	err_file_notfound	equ 04h
	err_goto_notlabel	equ 05h
	err_label_notfound	equ 06h
	err_nest_limit		equ 07h
	err_next			equ 08h
	err_loop			equ 09h
	err_print_type		equ 10h
	err_quote_term		equ 11h
	err_return			equ 12h
	err_string_range	equ 13h
	err_syntax			equ 14h
...
;	variables		times 26 dw 0	; Storage space for variables A to Z
	variables		equ 32768-1024-384-52	; Storage space for variables A to Z

;	for_variables		times 26 dw 0	; Storage for FOR loops
	for_variables		equ 32768-1024-384-104	; Storage for FOR loops
;	for_code_points		times 26 dw 0	; Storage for code positions where FOR loops start
	for_code_points		equ 32768-1024-384-156	; Storage for code positions where FOR loops start
	
;	do_loop_store		times 10 dw 0	; Storage for DO loops
	do_loop_store		equ 32768-1024-384-176	; Storage for DO loops
	...
	gosub_depth		db 0
	gosub_points		equ 32768-1024-384-196	; Points in code to RETURN to
